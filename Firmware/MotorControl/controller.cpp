...
bool Controller::update() {
    /**从 Encoder 对象处实时获取并验证电机位置估计和速度估计值，pos_estimate_linear_src_，
    ...
    if (axis_->step_dir_active_) {
        if (config_.circular_setpoints) {
            /**pos_wrap 用于环形运动（circular setpoints）将位置值包裹在固定范围内，实现位置环的循环范围或周期长度，从而避免数值直线累加溢出或漂移。
             * 例如旋转轴，位置可以在 0~360 无限循环，本质就是把位置值限制在 [0, pos_wrap) 区间*/
    ...
    if (config_.circular_setpoints) {
        ...
        /*使 input_pos_ 保持在 0~pos_wrap（或指定范围）以内，实现位置循环范围，避免角度漂移*/
